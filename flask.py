...
class Flask:
	...
	def __init__(self, warn = True):
		...
		self.autogen_ocaml = "(* This file is automatically generated.  Do not edit. *)\n"
		self.commons   = []
		self.user_commons = []
		self.common    = {}
		self.classes   = []
	...
	def createOcaml(self, path):
		'''
		Creates the Ocaml modules in the given PATH.
		'''
		files = {
			'flask_gen.ml' : self.createFlaskOcaml()
		}
	...
	def createOcamlL(self, count):
		fields = [1, 2, 4, 8]
		return "0x%08xl" % (fields[count % 4] << 4 * (count / 4))
	...
	def createFlaskOcaml(self):
		results = []
		results.append(self.autogen_ocaml)

		results.append("\n(* Security Classes *)\n")
		results.append("module Class = struct\n")
		count = 1
		for c in self.classes:
			results.append("  let %s = %dl\n" % (c, count))
			count += 1
		results.append("\n")

		results.append("  let class_to_string = [\n")
		count = 1
		for c in self.classes:
			results.append("    (%dl, \"%s\");\n" % (count, c))
			count += 1
		...
		results.append("module Perm = struct\n")
		for c in self.vectors:
			count = 0
			...
			ps += self.vector[c]
			for p in ps: 
				results.append("  let %s__%s = %s\n" % (c, p, self.createOcamlL(count)))
				count += 1
		results.append("\n")

		results.append("  let perm_to_string = [\n")
		class_id = 1
		for c in self.vectors:
			results.append("    (%dl, [\n" % class_id)
			perm_id = 0
		...
		results.append("\n(* Initial SIDs *)\n")
		results.append("module InitialSID = struct\n")
		count = 0
		for s in self.sids:
			count += 1
			results.append("  let %s = %dl\n" % (s, count))
		results.append("\n")
		results.append("  let to_string = [\n")
		count = 0
		for s in self.sids:
			count += 1
			results.append("    (%dl, \"%s\");\n" % (count, s))
...
def usage():
	...
	usage += '    --ocaml\toutput ocaml definitions\n'
	return usage
...
if __name__ == '__main__':
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], 'a:i:s:o:kuwh', ['access_vectors=', 'initial_sids=', 'security_classes=', 'output=', 'kernel', 'user', 'nowarnings', 'ocaml', 'help'])
